...
from vlpretrain2.concept_swapping import ConceptGenerator
from vlpretrain2.concept_replacing import ConceptReplacer
...
class ImgSentDataset:
    def __init__(self, args, img_splits: str, lang_splits: str, tiny=False, fast=False, val=False):
        """
...
class ImgSentTorchDataset(Dataset):
    def __init__(self,
        ...
        if perturb == 'swap':
            self.generator = ConceptGenerator(args.randomnum, args.k)
        else:
            self.replacer = ConceptReplacer(val, wiki=args.wiki)
    ...
    def __getitem__(self, item: int):
        ...
        if self.perturb == 'swap':
            generated_sent, shuffled_id, shuffled_tok = self.generator.generate(sent)
        else:
            generated_sent, shuffled_id, shuffled_tok = self.replacer.generate(sent)
        ...
        # tokens_pre = self.tokenizer.basic_tokenizer.tokenize(generated_sent)
        # assert len(tokens_pre) == len_tokens


        tokens, mlm_labels = self.get_subtoken_labels(tokens_pre, shuffled_id, shuffled_tok)

        # convert token to its vocab id
        input_ids = self.tokenizer.convert_tokens_to_ids(tokens)
        # truncate
        if len(input_ids) > self.sent_len:
            input_ids = input_ids[:self.sent_len]
            mlm_labels = mlm_labels[:self.sent_len]

        input_mask = [1] * len(input_ids)

        # Zero-pad up to the sequence length.
        while len(input_ids) < self.sent_len:
            input_ids.append(0)
            input_mask.append(0)
            mlm_labels.append(-1)
        ...
        return uid, (torch.tensor(input_ids), torch.tensor(input_mask)), torch.tensor(mlm_labels), (tensor_img, ), sent, generated_sent
        # return uid, (input_ids, attention_mask, ), (neg_input_ids, neg_attention_mask, ), (tensor_img, )
    
    def get_subtoken_labels(self, tokens, shuffled_id, shuffled_tok):
        ...
        for i, token in enumerate(tokens):
            ...
            for sub_token in sub_tokens:
                ...
                # if token.lower().strip() in shuffled_tok:
                if i in shuffled_id and token.lower().strip() in shuffled_tok:
