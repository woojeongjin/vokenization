...
from vlpretrain2.data import ImgSentDataset, ImgSentTorchDataset
from vlpretrain2.loss import paired_hinge_rank_loss2, binary_classification_loss
from vlpretrain2.metric import batchwise_accuracy, batchwise_recall
from vlpretrain2.model import LangModel, VisnModel, JointModel, LANG_MODELS
from vlpretrain2.param import parse_args

# import neptune.new as neptune
neptune=False
if neptune:
    run = neptune.init(project='woojeong/vl-pretraining')
...
def main():
    ...
    if neptune:
        run['parameters'] = {"learning_rate": args.lr, 'visn': args.visn, 'bertonly': args.bertonly}
...
def train(gpu, args):
    ...
    lang_model = LangModel(args.dim, arch=args.lang, layers=lang_layers,
                           pretrained=args.lang_pretrained, finetuning=args.lang_finetune, bertonly=args.bertonly, nonorm=args.nonorm)
    visn_model = VisnModel(args.dim, arch=args.visn,
                           pretrained=args.visn_pretrained, finetuning=args.visn_finetune, bertonly=args.bertonly, nonorm=args.nonorm)
    # The use of joint model would help synchronization in distributed learning.
    model = JointModel(args.dim, lang_model, visn_model, bertonly=args.bertonly)
    ...
    path = '/home/woojeong2/vok_pretraining/snap/vlpretrain2'
    if os.path.exists(os.path.join(path, 'trainset.pkl')):
        train_set = pickle.load(open(os.path.join(path, 'trainset.pkl'), 'rb'))

    # Data Sets
    else:
        train_set = ImgSentDataset(args, args.train_imgs, args.train_langs, tiny=args.tiny, fast=args.fast)
        pickle.dump(train_set, open(
            os.path.join(path, 'trainset.pkl'), 'wb'))
    ###
    # train_set = ImgSentDataset(args, args.train_imgs, args.train_langs, tiny=args.tiny, fast=args.fast)
    ###
    train_tset = ImgSentTorchDataset(args,
        train_set, train_transform, tokenizer, max_len, perturb=args.perturb
    )
    print("GPU %d: load %d data in training." % (gpu, len(train_set)))

    if os.path.exists(os.path.join(path, 'validset.pkl')):
        valid_set = pickle.load(open(os.path.join(path, 'validset.pkl'), 'rb'))

    # Data Sets
    else:
        valid_set = ImgSentDataset(args, args.valid_imgs, args.valid_langs, tiny=args.tiny, fast=args.fast)
        pickle.dump(valid_set, open(
            os.path.join(path, 'validset.pkl'), 'wb'))
    ###        
    # valid_set = ImgSentDataset(args, args.valid_imgs, args.valid_langs, tiny=args.tiny, fast=args.fast, val=True)
    ###
    # valid_set.shuffle()         # Valid set only gets shuffled once!!!
    print("GPU %d: load %d data in validation." % (gpu, len(valid_set)))
    valid_tset = ImgSentTorchDataset(args,
    ...
    for epoch in range(args.epochs):
        ...
        for i, (uid, lang_input, labels, visn_input, s, g) in enumerate(tqdm.tqdm(train_loader, disable=(gpu!=0))):
            # Currently, lang_input is the (input_ids, attention_mask)
            # visn_input is (tensor_img)
            # print(labels[0], s[0], ',',g[0])
            ...
            # Forward pass
            model.zero_grad()
            output = model(lang_input, visn_input)
        ...
        if gpu == 0:
            ...
            if results['loss'] < best_valid_loss:
                ...
                torch.save(model.module.lang_model.state_dict(), snap_path+'_lang')
                torch.save(model.module.lang_model, snap_path + '.model_lang')
            if neptune:
                run["best_valid_loss"].log(best_valid_loss)
...
def valid(args, model, criterion, valid_loader, use_tqdm=True):
    ...
    for i, (uid, lang_input, labels, visn_input, sent, neg_sent) in enumerate(iterator):
        # Currently, lang_input is the (input_ids, attention_mask)
        # visn_input is (tensor_img)
        lang_input = tuple(x.cuda(non_blocking=True) for x in lang_input)
        # neg_lang_input = tuple(x.cuda(non_blocking=True) for x in neg_lang_input)
        ...
        with torch.no_grad():
            ...
            # Evaluation
            results['loss'] += criterion(output, labels).item()
            acc_result, size, pos, neg = batchwise_accuracy(output, labels)
            acc_result_fn, size_fn, _, _ = batchwise_accuracy(output, labels, fn=True)
            acc_result_strict, size_strict, _, _ = batchwise_accuracy(output, labels, strict=True)
            results['acc'] += acc_result.item()
            results['acc_fn'] += acc_result_fn.item()
            results['acc_strict'] += acc_result_strict
            total_size_strict += size_strict
            total_size += size
            total_size_fn += size_fn
            pos_size += pos
            neg_size += neg
                # results['R%d' % key] += value
    print("pos", pos_size, "neg", neg_size)
    print("ratio", pos_size.item()/(pos_size.item()+neg_size.item()))
    ratio = pos_size.item()/(pos_size.item()+neg_size.item())
    if neptune:
        run['ratio'].log(ratio)
    for key in results:
        if key == 'acc':
            ...
            results[key] = results[key] / total_size_fn
        elif key == 'acc_strict':
            results[key] = results[key] / total_size_strict
        else:
