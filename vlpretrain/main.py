...
from vlpretrain.data import ImgSentDataset, ImgSentTorchDataset
from vlpretrain.loss import paired_hinge_rank_loss2, binary_classification_loss
from vlpretrain.metric import batchwise_accuracy, batchwise_recall
...
def train(gpu, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (uid, lang_input, neg_lang_input, visn_input, _, _) in enumerate(tqdm.tqdm(train_loader, disable=(gpu!=0))):
...
def valid(args, model, criterion, valid_loader, use_tqdm=True):
    ...
    for i, (uid, lang_input, neg_lang_input, visn_input, sent, neg_sent) in enumerate(iterator):
        # Currently, lang_input is the (input_ids, attention_mask)
        # visn_input is (tensor_img)
        tem = lang_input
        lang_input = tuple(x.cuda(non_blocking=True) for x in lang_input)
        ...
        with torch.no_grad():
            ...
            # Evaluation
            results['loss'] += criterion(lang_output, visn_output, neg_lang_output, neg_visn_output, args.margin, args.bertonly).item()
